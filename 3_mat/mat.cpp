...
int main( int argc, const char** argv )
{
	...
	switch(sample)
	{
		...
		case 4:
		{
			...
			cout << "elemSize1: " << image.elemSize1() << endl;


			int myRow=511;
			int myCol=511;
			int val=*(image.data+myRow*image.cols*image.channels()+ myCol*image.channels());
...
//			waitKey(0);
		...
		case 5:
		{
			...
			int myRow=511;
			int myCol=511;
			int B=*(image.data+myRow*image.cols*image.channels()+ myCol*image.channels() + 0);
			int G=*(image.data+myRow*image.cols*image.channels()+ myCol*image.channels() + 1);
			int R=*(image.data+myRow*image.cols*image.channels()+ myCol*image.channels() + 2);
			cout << "Pixel value (B,G,R): (" << B << "," << G << "," << R << ")" << endl;
//			imshow("Lena", image);
//			waitKey(0);
			imwrite("Lena_re.jpg", image);
			break;
		}
		case 6:
		{
